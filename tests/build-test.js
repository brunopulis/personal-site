...
const EXPECTED_ASSETS = ['assets/css/build.css', 'assets/js/search.js', 'assets/js/easter-eggs.js'];
...
function runTests() {
  ...
  test('Blog posts have microformats', () => {
    try {
      ...
      return content.includes('h-entry') && content.includes('p-name') && content.includes('dt-published');
    } catch (e) {
      return false;
    }
  });
  ...
  test('Sitemap contains expected pages', () => {
    try {
      ...
      return (
        content.includes('Site Directory') &&
