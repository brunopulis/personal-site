...
export async function GET(context) {
  ...
  const sortedBooks = books.sort((a, b) => {
    return (
      new Date(b.data.reading_date).getTime() -
      new Date(a.data.reading_date).getTime()
    );
  });
  ...
  return rss({
    ...
    items: sortedBooks.map((book) => {
      ...
      if (book.data.rating && book.data.status === 'Lido') {
        ...
        parts.push(
          `<strong>Avaliação:</strong> ${stars} (${book.data.rating}/5)`
        );
      }
      ...
      if (book.data.category && book.data.category.length > 0) {
        parts.push(
          `<strong>Categorias:</strong> ${book.data.category.join(', ')}`
        );
      }
      ...
      if (book.data.quotes) {
        parts.push(
          `<strong>Citação destacada:</strong><br/><em>"${book.data.quotes}"</em>`
        );
      }
      ...
      if (book.data.recommended_for) {
        parts.push(
          `<strong>Recomendado para:</strong> ${book.data.recommended_for}`
        );
      }
      ...
      if (book.data.purchase_link) {
        parts.push(
          `<a href="${book.data.purchase_link}">🛒 Comprar este livro</a>`
        );
      }
