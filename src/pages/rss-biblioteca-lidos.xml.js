  ...
  return rss({
    ...
    items: readBooks.map((book) => {
      ...
      if (book.data.category && book.data.category.length > 0) {
        parts.push(
          `<br/><br/><strong>Categorias:</strong> ${book.data.category.join(', ')}`
        );
      }
      ...
      if (book.data.quotes) {
        parts.push(
          `<br/><br/><strong>Citação:</strong><br/><em>"${book.data.quotes}"</em>`
        );
      }
      ...
      if (book.data.recommended_for) {
        parts.push(
          `<br/><br/><strong>Recomendado para:</strong> ${book.data.recommended_for}`
        );
      }
      ...
      if (book.data.purchase_link) {
        parts.push(
          `<br/><br/><a href="${book.data.purchase_link}">📖 Comprar este livro</a>`
        );
      }
